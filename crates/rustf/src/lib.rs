...
use rustler::{Encoder, Env, Error, ListIterator, NifResult, ResourceArc, Term};
...
fn to_tensor(term: Term) -> NifResult<ResourceArc<TensorResource>> {
    let v: Vec<Vec<f32>> = term
        .decode::<ListIterator>()?
        .map(from_vector)
        .collect::<NifResult<Vec<Vec<f32>>>>()?;
    let t = matrix_to_tensor(v);
    Ok(ResourceArc::new(TensorResource {
        payload: RwLock::new(t),
    }))
}
...
fn transpose(res: ResourceArc<TensorResource>) -> NifResult<ResourceArc<TensorResource>> {
    ...
    let f = |t: Tensor<f32>| -> Result<Tensor<f32>, tensorflow::Status> {
        ...
        let _ = tensorflow::ops::Transpose::new()
            .T(Float)
            .Tperm(Int32)
            .build(in1, perm, &mut scope.with_op_name("transpose"))?;

        let mut step = tensorflow::SessionRunArgs::new();

        step.add_feed(&scope.graph().operation_by_name_required("input1")?, 0, &t);

        // fetch final result
        let result = step.request_fetch(&scope.graph().operation_by_name_required("transpose")?, 0);
    ...
    match f(t1.clone()) {
        Ok(val) => Ok(ResourceArc::new(TensorResource {
            payload: RwLock::new(val),
        })),
        Err(_) => Err(Error::RaiseAtom("null")),
    }
}
...
fn diag(res: ResourceArc<TensorResource>) -> NifResult<ResourceArc<TensorResource>> {
    ...
    let f = |t: Tensor<f32>| -> Result<Tensor<f32>, tensorflow::Status> {
        ...
        let _ = match t.dims().len() {
            1 => tensorflow::ops::Diag::new()
                .T(Float)
                .build(in1, &mut scope.with_op_name("diag"))?,

            2 => tensorflow::ops::DiagPart::new()
                .T(Float)
                .build(in1, &mut scope.with_op_name("diag"))?,
            _ => todo!(),
        };
        ...
        // fetch final result
        let result = step.request_fetch(&scope.graph().operation_by_name_required("diag")?, 0);
...
fn inv(res: ResourceArc<TensorResource>) -> NifResult<ResourceArc<TensorResource>> {
    ...
    let f = |t: Tensor<f32>| -> Result<Tensor<f32>, tensorflow::Status> {
        ...
        let _ = tensorflow::ops::Inv::new()
            .T(Float)
            .build(in1, &mut scope.with_op_name("inv"))?;
        ...
        // fetch final result
        let result = step.request_fetch(&scope.graph().operation_by_name_required("inv")?, 0);
...
#[rustler::nif]
fn matmul(
    res_a: ResourceArc<TensorResource>,
    res_b: ResourceArc<TensorResource>,
    ...
    let f = |a: Tensor<f32>, b: Tensor<f32>| -> Result<Tensor<f32>, tensorflow::Status> {
        ...
        step.add_feed(&scope.graph().operation_by_name_required("input1")?, 0, &a);

        step.add_feed(&scope.graph().operation_by_name_required("input2")?, 0, &b);

        // fetch final result
        let result = step.request_fetch(&scope.graph().operation_by_name_required("matmul")?, 0);
...
#[rustler::nif]
fn svd(
    res: ResourceArc<TensorResource>,
) -> NifResult<(
    ...
    let f =
        |t: Tensor<f32>| -> Result<(Tensor<f32>, Tensor<f32>, Tensor<f32>), tensorflow::Status> {
            let scope = Scope::new_root_scope();
            let session =
                tensorflow::Session::new(&tensorflow::SessionOptions::new(), &scope.graph())?;

            let in1 = tensorflow::ops::Placeholder::new()
                .dtype(Float)
                .build(&mut scope.with_op_name("input1"))?;

            let _ = tensorflow::ops::Svd::new()
                .T(Float)
                .build_instance(in1.into(), &mut scope.with_op_name("svd"))?;
            ...
            // fetch final result
            let s = step.request_fetch(&scope.graph().operation_by_name_required("svd")?, 0);

            let u = step.request_fetch(&scope.graph().operation_by_name_required("svd")?, 1);

            let v = step.request_fetch(&scope.graph().operation_by_name_required("svd")?, 2);
    ...
    match f(t1.clone()) {
        Ok((s, u, v)) => Ok((
            ResourceArc::new(TensorResource {
                payload: RwLock::new(s),
            }),
            ResourceArc::new(TensorResource {
                payload: RwLock::new(u),
            }),
            ResourceArc::new(TensorResource {
                payload: RwLock::new(v),
            }),
        )),
        Err(_) => Err(Error::RaiseAtom("null")),
    }
